#In this we compute two versions of shortest path betweenness
using LightGraphs

function betweenness_centrality_prot(option,g::AbstractGraph,rst::AbstractVector,src::AbstractVector=vertices(g),distmx::AbstractMatrix=LightGraphs.weights(g);normalize=true,   endpoints=false)
    ...
    if option == 1 #Baseline 1: Shortest-path metric considering only the shortest path between set source set S (considered as super node) (src) and P (rst or targets)
    	 state = dijkstra_shortest_paths(g, src, distmx; allpaths=true, trackvertices=true)
    	_accumulate_basic!(betweenness, state, g, src,rst)
    elseif option == 2 #Baseline 2: Shortest-path metric considering all shortest paths between all vertices of set S (src) and set P(rst or targets)
   	for s in src
	...
	btw=removesrc_vert_p(betweenness,rst,src)
	return btw
#    return betweenness
end
